...
public sealed class PaperShortPositionServiceTests
{
    ...
    public void Full_cover_closes_position_and_records_profit()
    {
        ...
        var position = DurablePaperShortPosition.Open(Guid.NewGuid(), Guid.NewGuid(), new Symbol("TEST"), 5, 100m, now);
    ...
    {
        var now = DateTimeOffset.UtcNow;
        var position = DurablePaperShortPosition.Open(Guid.NewGuid(), Guid.NewGuid(), new Symbol("TEST"), 10, 100m, now);
    ...
    {
        var now = DateTimeOffset.UtcNow;
        var position = DurablePaperShortPosition.Open(Guid.NewGuid(), Guid.NewGuid(), new Symbol("TEST"), 2, 100m, now);
