...
internal static class PersistenceMapping
{
    ...
    public static HistoricalCandleState ToState(this HistoricalCandleRecord r) => new(r.Id, new Symbol(r.Symbol, string.IsNullOrEmpty(r.InstrumentToken) ? null : r.InstrumentToken), r.Interval, r.Timestamp, r.Open, r.High, r.Low, r.Close, r.Volume, r.Source, r.ReceivedAt);
}
...
public sealed class EfOrderRepository(TradingDbContext db) : IOrderRepository
{
    ...
    public async Task<OrderState?> GetByIdempotencyKeyAsync(string key, CancellationToken ct) => await db.Orders.AsNoTracking().SingleOrDefaultAsync(x => x.IdempotencyKey == key, ct) is { } r ? r.ToState() : null;
    public Task AddAsync(OrderState o, CancellationToken ct) { db.Orders.Add(new OrderRecord { Id=o.Id, IdempotencyKey=o.IdempotencyKey, Symbol=o.Symbol.Value, InstrumentToken=o.InstrumentToken ?? o.Symbol.InstrumentToken, Side=o.Side.ToString(), Quantity=o.Quantity, LimitPrice=o.LimitPrice, StrategyVersion=o.StrategyVersion, CreatedAt=o.CreatedAt, ExecutionMode=o.ExecutionMode, Status=o.Status }); return Task.CompletedTask; }
...
{
    public Task AddRangeAsync(IReadOnlyList<HistoricalCandleState> candles, CancellationToken ct) { foreach (var c in candles) db.HistoricalCandles.Add(new HistoricalCandleRecord { Id=c.Id, Symbol=c.Symbol.Value, InstrumentToken=c.Symbol.InstrumentToken ?? string.Empty, Interval=c.Interval, Timestamp=c.Timestamp, Open=c.Open, High=c.High, Low=c.Low, Close=c.Close, Volume=c.Volume, Source=c.Source, ReceivedAt=c.ReceivedAt }); return Task.CompletedTask; }
