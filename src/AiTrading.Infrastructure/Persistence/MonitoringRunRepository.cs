...
public sealed class EfMonitoringRunRepository(IDbContextFactory<TradingDbContext> contextFactory) : IMonitoringRunRepository
{
    public async Task AddAsync(MonitoringRunState run, CancellationToken cancellationToken)
    {
        ...
        db.MonitoringRuns.Add(new MonitoringRunRecord
        {
            Id = run.Id,
    ...
    {
        await using var db = await contextFactory.CreateDbContextAsync(cancellationToken);
        var record = await db.MonitoringRuns.SingleAsync(x => x.Id == id, cancellationToken);
    ...
    {
        await using var db = await contextFactory.CreateDbContextAsync(cancellationToken);
        return await db.MonitoringRuns.AsNoTracking().Where(x => x.Id == id).Select(x => new MonitoringRunState(x.Id, x.StartedAt, x.CompletedAt, Enum.Parse<MonitoringRunStatus>(x.Status), x.PositionCount, x.FailureCount)).SingleOrDefaultAsync(cancellationToken);
    }
    ...
    public async Task<IReadOnlyList<MonitoringRunState>> GetRecentAsync(int limit, CancellationToken cancellationToken)
    {
        ...
        await using var db = await contextFactory.CreateDbContextAsync(cancellationToken);
        return await db.MonitoringRuns.AsNoTracking().OrderByDescending(x => x.StartedAt).Take(limit).Select(x => new MonitoringRunState(x.Id, x.StartedAt, x.CompletedAt, Enum.Parse<MonitoringRunStatus>(x.Status), x.PositionCount, x.FailureCount)).ToListAsync(cancellationToken);
    }
}
