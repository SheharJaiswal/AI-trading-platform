...
public sealed class InMemoryAlertStore : IAlertStore
{
    ...
    public bool TryAdd(Alert alert) { lock (_gate) return _alerts.TryAdd(alert.Key, alert); }
    public IReadOnlyList<Alert> GetAll() { lock (_gate) return _alerts.Values.OrderByDescending(x => x.CreatedAt).ToArray(); }
}
...
public sealed class RecommendationService(IMarketDataProvider marketData, MarketDataFreshnessOptions? freshness = null, DeterministicRecommendationEngine? engine = null)
{
    private readonly MarketDataFreshnessOptions _freshness = freshness ?? MarketDataFreshnessOptions.Default;
    private readonly DeterministicRecommendationEngine _engine = engine ?? new();
    ...
    public async Task<Recommendation> GetRecommendationAsync(Symbol symbol, CancellationToken cancellationToken)
    {
        ...
        return _engine.Evaluate(symbol, candles, quote.Timestamp);
    }
}
...
public sealed class RiskEngine
{
    ...
    {
        if (recommendation.RiskFactors.Contains("STALE_OR_INVALID_MARKET_DATA")) return new(RiskDecision.InsufficientData, "Market data is stale or invalid.");
...
public sealed class PaperPortfolio(decimal startingCash) : IPortfolio
{
    ...
    {
        var unrealized = _positions.Values.Sum(position => { var marketPrice = _marketPrices.GetValueOrDefault(position.Symbol, position.AverageEntryPrice); return (marketPrice - position.AverageEntryPrice) * position.Quantity; });
    ...
    public void Apply(Fill fill)
    {
        ...
        if (fill.Side == OrderSide.Buy)
        {
            ...
            _cash -= value;
            if (_positions.TryGetValue(fill.Symbol, out var existing)) { var quantity = existing.Quantity + fill.Quantity; var average = ((existing.AverageEntryPrice * existing.Quantity) + value) / quantity; _positions[fill.Symbol] = existing with { Quantity = quantity, AverageEntryPrice = average }; }
        ...
        {
            if (!_positions.TryGetValue(fill.Symbol, out var position) || position.Quantity < fill.Quantity) throw new InvalidOperationException("Cannot sell more than the open position.");
            _cash += value; _realized += (fill.Price - position.AverageEntryPrice) * fill.Quantity;
            var remaining = position.Quantity - fill.Quantity;
            if (remaining == 0) { _positions.Remove(fill.Symbol); _marketPrices.Remove(fill.Symbol); } else _positions[fill.Symbol] = position with { Quantity = remaining };
        }
    }
    public void SetStopLoss(Guid positionId, decimal stopLoss) { var match = _positions.FirstOrDefault(x => x.Value.Id == positionId); if (match.Value is null) throw new KeyNotFoundException("Position not found."); _positions[match.Key] = match.Value with { StopLoss = stopLoss }; }
    public void UpdateMarketPrice(Symbol symbol, decimal price) { if (_positions.ContainsKey(symbol)) _marketPrices[symbol] = price; }
}