...
/// <summary>Durable paper execution boundary. The supplied orderId is the idempotency key for a client execution attempt.</summary>
...
{
    public async Task<(RiskResult Risk, FillState? Fill)> ExecuteAsync(
        Guid portfolioId,
        Guid orderId,
        Symbol symbol,
        int quantity,
        CancellationToken cancellationToken)
    {
        ...
        var existingOrder = await unitOfWork.Orders.GetAsync(orderId, cancellationToken);
        ...
        if (riskResult.Decision != RiskDecision.Approved)
        ...
        var orderState = new OrderState(
            order.Id, order.Symbol, order.Symbol.InstrumentToken, order.Side, order.Quantity,
            order.LimitPrice, "baseline-v1", order.CreatedAt, "paper", "filled");
        var fillState = new FillState(
            fill.Id, fill.OrderId, fill.Symbol, fill.Side, fill.Quantity, fill.Price, fill.Timestamp, "paper");
        ...
        {
            await unitOfWork.Portfolios.SavePositionAsync(
                new PositionState(
                    Guid.NewGuid(), portfolioId, symbol, symbol.InstrumentToken, fill.Quantity,
                    fill.Price, fill.Price, null, now, now), cancellationToken);
        }
        else
        {
            ...
            await unitOfWork.Portfolios.SavePositionAsync(
                existingPosition with
                {
                    ...
                    CurrentMarketPrice = fill.Price,
                    UpdatedAt = now
                }, cancellationToken);
        }

        await unitOfWork.Portfolios.SaveAsync(
            portfolio with { Cash = portfolio.Cash - value, UpdatedAt = now },
            portfolio.Version,
            cancellationToken);
