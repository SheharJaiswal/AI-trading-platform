...
{
    public async Task<PredictionRecord> RecordPredictionAsync(
        string symbol,
        string horizon,
        decimal expectedReturn,
        decimal probabilityPositive,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var prediction = new PredictionRecord(
            Guid.NewGuid(), symbol.Trim().ToUpperInvariant(), horizon.Trim(), expectedReturn,
            probabilityPositive, confidence, modelVersion.Trim(), dataTimestamp, DateTimeOffset.UtcNow);
    ...
    public async Task RecordOutcomeAsync(PredictionOutcome outcome, CancellationToken cancellationToken)
    {
        ...
        if (outcome.OutcomeTimestamp < DateTimeOffset.MinValue) throw new ArgumentException("outcomeTimestamp is invalid.");
        var evaluations = await repository.GetEvaluationsAsync(null, cancellationToken);
        var prediction = evaluations.FirstOrDefault(x => x.PredictionId == outcome.PredictionId);
        if (prediction is not null && outcome.OutcomeTimestamp < prediction.DataTimestamp)
    ...
    public async Task<EvaluationMetrics> GetMetricsAsync(string? symbol, CancellationToken cancellationToken)
    {
        ...
        return new EvaluationMetrics(
            ...
            wins,
            losses,
            evaluated.Length == 0 ? 0m : (decimal)evaluated.Count(x => x.DirectionCorrect) / evaluated.Length,
            cumulative,
            maxDrawdown,
            cumulative);
    }
