...
public sealed class PaperTradingSessionExecutionService(IPaperTradingSessionService sessions, IPaperTradeService paperTrades)
{
    public async Task<PaperTradingEventResponse> ProcessAsync(PaperTradingEventRequest request, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(request.EventId) || request.EventId.Length > 128)
            throw new ArgumentException("EventId is required and must be 1-128 characters.", nameof(request));
