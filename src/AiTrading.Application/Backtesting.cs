...
public sealed class DeterministicBacktestEngine(DeterministicRecommendationEngine recommendationEngine, RiskEngine riskEngine)
{
    public BacktestResult Run(Symbol symbol, IReadOnlyList<HistoricalCandle> historicalCandles, BacktestConfiguration configuration)
    {
        ...
        var cash = configuration.StartingCash; var quantity = 0; var averageEntry = 0m;
        var trades = new List<BacktestTrade>(); var riskEvents = new List<BacktestRiskEvent>(); var curve = new List<BacktestEquityPoint>();
        var peak = configuration.StartingCash; var maxDrawdown = 0m;
        for (var i = 0; i < ordered.Length; i++)
        {
            ...
            if (risk.Decision == RiskDecision.Approved)
            {
                ...
                var gross = fillPrice * configuration.QuantityPerTrade; var fee = gross * configuration.FeeRate; var total = gross + fee;
                if (total <= cash)
                {
                    var oldValue = averageEntry * quantity; quantity += configuration.QuantityPerTrade;
                    averageEntry = quantity == configuration.QuantityPerTrade ? fillPrice : (oldValue + gross) / quantity;
                    cash -= total; trades.Add(new(candle.Timestamp, OrderSide.Buy, configuration.QuantityPerTrade, fillPrice, fee, risk.Decision, risk.Reason));
                }
            }
            var positionValue = quantity * candle.Close; var equity = cash + positionValue; peak = Math.Max(peak, equity);
            if (peak > 0) maxDrawdown = Math.Max(maxDrawdown, (peak - equity) / peak * 100m);
            curve.Add(new(candle.Timestamp, cash, positionValue, equity));
        }
        var ending = curve[^1].Equity; var returnPercent = (ending - configuration.StartingCash) / configuration.StartingCash * 100m;
